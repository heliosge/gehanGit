	...
	$('#cjylTable').mmGrid(
					{
						...
						url : '<%=request.getContextPath()%>/reportForms/getGradeDetailByDept.action',
						width : '1064px',
						height : 'auto',
						...
						cols : [
								...
								{title : '岗位',name : 'post',align : 'center'},
								{title : '考试名称',name : 'examTitle',align : 'center'},
								{title : '考试时间',name : 'startTime',width : 210,align : 'center',
									renderer : function(val, item, rowIndex) {
										...
										str = item.startTime+" - "+item.endTime
										return str;
									}
								},
								{title : '你的得分/总分/及格分',name : 'getScore',align : 'center',
									renderer : function(val, item, rowIndex) {
										return val + "/" + item.totalScore + "/" + item.passScore;
									}
								},
								{title : '是否通过',name : 'isPassed',align : 'center', width:50,
									renderer : function(val, item, rowIndex) {
										if(val == 1){
											return "是";
										}else{
