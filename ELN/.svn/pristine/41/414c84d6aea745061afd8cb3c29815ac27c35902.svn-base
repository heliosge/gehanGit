...
/**分类标题*/
#typesDiv{margin-left: 20px;font-size: 14px;width: 1046px;min-height: 40px;height:auto;}
...
/**分类菜单样式*/
.menuType{padding-left: 0px;}

.menuType ul{list-style: none;padding-left:0px;}

.menuType li{list-style: none;float: left;}
</style>
...
function initTypeName(){
	$.ajax({
		...
		async:true,//默认异步
		data:{"typeId":typeId},
		url:'<%=request.getContextPath()%>/myAsk/getTypeById.action',
		success:function(data){
			if(data){
				$("#searchAllAsks").append('&gt;');
				var htmlStr = '';
				htmlStr += '<li>';
				htmlStr += '<ul>';
				htmlStr += '<li>&nbsp;<a id="'+data.id+'" href="javascript:void(0);" onclick="searchTypesAndAsks(this);">'+data.name+'</a>&nbsp;</li>';
				htmlStr += '</ul>';
				htmlStr += '</li>';
				$("#menuTypeUl").append(htmlStr);
			}
		}
	});
}
...
function initTypeAsksForMMGrid(){
	...
	$("#askListTable").mmGrid({
		...
		url : '<%=request.getContextPath()%>/myAsk/getTypeAsksForMMGrid.action',
		width : '1046px',
		height : 'auto',
 ...
 * 查询该分类下的所有直接子分类
 */
function searchTypesAndAsks(obj){
	$.ajax({
		type:'POST',
		async:false,//此处为同步
		data:{"typeId":$(obj).attr("id"),"companyId":companyId,"subCompanyId":subCompanyId},
		url:'<%=request.getContextPath()%>/myAsk/getSearchTypes.action',
		success:function(data){
			if(data != null && data.length > 0){
				//有子分类
				//1、清空该li后面的所有li
				$(obj).parent().addClass("thisTypeStyle");
				$(obj).parent().parent().find("li").not(".thisTypeStyle").remove();
				$(obj).parent().removeClass("thisTypeStyle");
				//如果已经在li里已经有>号，就不再添加
				var liStr = $(obj).parent().html();
				if(liStr.indexOf("&gt;") < 0){
					$(obj).parent().append('&gt;');
				}
				//2、添加新的ul和li
				...
				for(var i = 0; i < data.length; i++){
					menuStr += '<li>&nbsp;<a id="'+data[i].id+'" href="javascript:void(0);" onclick="searchTypesAndAsks(this);">'+data[i].name+'</a>&nbsp;</li>';
				}
				menuStr += '</ul>';
				menuStr += '</li>';
				$(obj).parent().parent().append(menuStr);
				//3、查询该分类下的所有问题
				$("#searchContent").val('');
				var searchParam = {};
				searchParam.typeId = $(obj).attr("id");
				$("#askListTable").mmGrid().load(searchParam);
			}else{
				//无子分类
				//查询该分类下的所有问题
...
<body>
	<div id="askTypeDetailFrame">
		<div style="padding-bottom: 5px;border-bottom: 1px solid #cccccc; margin-bottom: 10px;">
			...
			<span style="  position: relative;top: 5px; color: #3a3a3a; font-size: 16px; font-weight: bold;">分类详情</span>
		</div>
		<!-- <h3><a href="javascript:void(0)" onclick="backLastPage();">&lt;</a>&nbsp;&nbsp;&nbsp;分类详情</h3> -->
		<div id="typesDiv">
			<ul id="menuTypeUl" class="menuType">
				<li id="searchAllAsks">&nbsp;<a href="javascript:void(0);" onclick="searchTypesAndAsks(this);">全部问题</a>&nbsp;</li>
				<!-- <li>
					<ul>
						<li>&nbsp;<a href="javascript:void(0);">ceshi001</a>&nbsp;</li>
						<li>&nbsp;<a href="javascript:void(0);">ceshi002</a>&nbsp;</li>
					</ul>
				</li> -->
			</ul>
			<!-- 全部问题 > <em id="typeName"></em> -->
		</div>
		<!-- 搜索框 -->
		<div id="searchDiv">
