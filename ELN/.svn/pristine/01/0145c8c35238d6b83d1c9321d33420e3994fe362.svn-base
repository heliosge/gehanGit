...
<script>
	...
	function initGrid(){
		...
		$('#exampleTable').mmGrid({
			...
			url:"<%=request.getContextPath()%>/train/selectTrainPlanCheckList.action",
	        ...
	        params: function(){
	     	...
	     	cols: [{title: 'ID', name: 'id', width:30, hidden:true},
	               {title: '培训计划', name: 'plan', width:120, align:'center', renderer:function(val, item, rowIndex){
	 				  return val.timeValue+'--'+val.name;
				   }},
	 			   {title: '提交人', name: 'plan', width:60, align:'center', renderer:function(val, item, rowIndex){
					   return val.submitUserName;
				   }},
	 			   {title: '提交时间', name: 'plan', width:60, align:'center', renderer:function(val, item, rowIndex){
	 				  return getSmpFormatDateByLong(val.updateTime, true);
				   }},
	 			   {title: '提交部门', name: 'plan', width:60, align:'center', renderer:function(val, item, rowIndex){
					   return val.submitDept;
				   }},
				   {title: '状态', name: 'status', width:60, align:'center', renderer:function(val, item, rowIndex){
					   return val==1?'待审批':(val==2?'审批通过':'审批拒绝');
				   }},
				   {title: '操作', name: 'id', width:120, align:'center', renderer:function(val, item, rowIndex){
					   
					   var buttonStr ='';
					   if(item.status == 1){
						   buttonStr += '<a href="javascript:void(0);" onclick="checkPlanDialog('+item.id+')" >审批</a> ';
					   }else{
						   buttonStr += '<a href="javascript:void(0);" style="color:#999;">审批</a> ';
					   }
					   buttonStr += '<a href="toTrainPlanCheckDetail.action?id='+val+'">详情</a>';
					   return buttonStr;
				   }}
	           ],
	        plugins : [
...
<body>
	...
	<div class="content">
		...
		<div style="padding-bottom: 5px;border-bottom: 1px solid #cccccc; margin-bottom: 10px;">
			...
			<span id="title_span" style="  position: relative;top: 5px; color: #3a3a3a; font-size: 16px; font-weight: bold;">培训计划审批</span>
		</div>
		<div class="ul_know" id="kn_ce">
		 ...
		 <div class="btn_gr">
        <input type="button" class="btn_1" value="批量审批" onclick="checkPlansDialog()"/>
    </div>
    
	<div class="search_2">
			<input type="hidden" id="timeType" value="1"/>
        	<p><span id="timeTypeSpan">年度：</span>
                    <select id="timeValue">
	...
	<div class="search_2">
		<p>
            	...
            	提交部门：<input type="text" id="submitDept"/>
                   	状态：
                    <select id="status">
