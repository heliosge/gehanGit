...
$(function(){
	...
	$('#examTable').mmGrid({
		...
		url:'<%=request.getContextPath()%>/myExercise/getExerciseWrongCard.action',
		width: '1046px',
		height: 'auto',
	    ...
	    cols: [{title: 'ID', name: 'wrongId',hidden:true},
	           {title:'题干', name:'content', width:250,align:'center', 
	        	   renderer:function(val,item, rowIndex){
				       return "<a class='question_img' href='javascript:void(0);' onclick='viewWrongQuestion("+item.wrongId+")'>"+val+"</a>";
	        	   }
               },
              ...
              {title:'试题分类',name:'categoryName', width:150,align:'center'},
               {title:'操作',name:'operation', width:155,align:'center',
                	renderer:function(val,item, rowIndex){
                		var str_enter = '<a href="javascript:void(0);" onclick="exerciseAgain('+ item.wrongId + ')" >再次练习</a>&nbsp;';
                		return str_enter;
                	}	
	}] ,
	   plugins : [
...
/*移出错题集  */
function exerciseAgain(wrongId,answerId){
	dialog.confirm('确认再次练习吗？', function(){
		$.ajax({
			type:"POST",
...
/*查看错题详情  */
function viewWrongQuestion(wrongId){
	window.location = '<%=request.getContextPath()%>/myExercise/gotoExerciseWrongDetail.action?id='+wrongId+"&wrongType=3";
}

/*再次练习  */
function exerciseAgain(wrongId){
	window.location = '<%=request.getContextPath()%>/myExercise/gotoExerciseAgain.action?id='+wrongId;
}
...
$(function(){
	$('#ul_exam').find('li').click(function(){
		...
		//query();
		<%-- if($(this).attr("id") == 1){
			window.location = '<%=request.getContextPath()%>/myExamManage/gotoWrongCard.action';
		}else  --%>
		if($(this).attr("id") == 2){
...
<input type="hidden" id="wrongType" name="wrongType" value="3"/>
<div id="course_all">
