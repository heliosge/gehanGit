...
public class PropertyUtil {
	...
	/** 读取src下database.properties配置文件内容 */
	private final static String DATABASE_LOCATION = "database.properties";
	/** 接收Properties内容 */
	...
	/**
	 * Method name: getDatabaseProperties <BR>
	 * Description: 读取src下第三方配置文件database.properties配置文件内容 <BR>
	 * Remark: <BR>
	 * @param propertyName
	 * @return String<BR>
	 */
	public static String getDatabaseProperties(String propertyName) {
		try {
			property.load(PropertyUtil.class.getClassLoader().getResourceAsStream(DATABASE_LOCATION));
			String result = property.getProperty(propertyName);
			if (null != result && !"".equalsIgnoreCase(result)) {
				return result.trim();
			} else {				
