...
$(function(){
	...
	$('#exampleTable').mmGrid({
		...
		url:"<%=request.getContextPath()%>/mall/manage/sellRecord/list.action",
        ...
        params: function(){
     	...
     	cols: [{title: 'ID', name: 'id', width:30, hidden:true},
 			  ...
 			  {title: '所属企业', name: 'companyName', width:30, align:'center'},
 			 {title: '购买企业', name: 'buyCompanyName', width:30, align:'center'},
 			  {title: '下单时间', name: 'createTime', width:80, align:'center'},
 			  {title: '付款状态', name: 'status', width:30, align:'center',renderer:function(val,item,rowIndex){
 				 var str='';
 				 if(val==1){
 					str='已付款' ;
 				 }else if(val==2){
 				 ...
 				 return str;
				   ...
				   {title: '操作', name: 'status', width:120, align:'center', renderer:function(val, item, rowIndex){
					   ...
					   if(val == 0){
							   ...
							   buttonStr += '<a href="javascript:void(0)" onclick="payOne('+item.id+')" >付款</a>  ';  
						}else if(val == 1){
							
								  buttonStr += '<a href="javascript:void(0)" style="color:grey" >付款</a> ';
						}
...
function renderInfo(data){
	...
	$("#payMoney").text(data.other.payMoney||0);
}
...
window.open("<%=request.getContextPath()%>/mall/manage/exportRecord.action?"+paramStr);
...
<div class="content">
	<div style="padding-bottom: 5px;border-bottom: 1px solid #cccccc; margin-bottom: 10px;">
		...
		<span style="  position: relative;top: 5px; color: #3a3a3a; font-size: 16px; font-weight: bold;">课程销售记录</span>
	 </div>
    ...
    <div class="right_lesson"  style="">
       <div class="lesson_tab" >
        	<ul>
            	<li ><a href="<%=request.getContextPath()%>/mall/manage/toSellRecordListPage.action" style="color:black;">普联课程销售记录</a></li>
                <li class="li_this">企业课程销售记录</li>
               
            </ul>
        </div>
        <div class="button_gr" style="height: 40px;line-height: 60px;">
        <h4 >课程销售总金额：¥<span id="totalMoney"></span>   课程总销量：<span id="totalRecord"></span>个 已付款金额：¥<span id="payMoney"></span></h4>
        </div>
        <div class="button_gr">
          <input type="button" value="导出记录" onclick="exportToExcel()"/>
            <input type="button" value="批量付款" onclick="pay()"/>
        </div>
        <div class="search_3">
        	<p>
            	 课程名称：
