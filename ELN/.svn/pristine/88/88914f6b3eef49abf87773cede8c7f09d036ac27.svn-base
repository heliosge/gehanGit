...
$(function(){
	...
	grid.on("loadSuccess",function(e, data){
		...
		for(var j=0;j<rows.length;j++){
			if(rows[j].examId==window.parent.$("#afterClassExam").val()){
				grid.select(j);
			}
		}
	});
...
function save(){
	...
	if(selectRows.length > 0){
		window.parent.$("#afterClassExam").val(selectRows[0].examId);
		window.parent.$("#afterClassExamName").val(selectRows[0].title);
	}
