 				 ...
 				 return str;
				   ...
				   {title: '操作', name: 'status', width:120, align:'center', renderer:function(val, item, rowIndex){
					   ...
					   if(val == 0){
						   ...
						   buttonStr += '<a href="javascript:void(0)" style="color:grey" >收款</a> ';
						}else if(val == 1){
							buttonStr += '<a href="javascript:void(0)" onclick="gatheringOne('+item.id+')" >收款</a>  ';  
...
function gathering(){
	...
	if(selectRows.length>0){
		...
		doGathering(ids.join(","));
	}else{
		dialog.alert("请至少选择一个课程销售记录");
	}
}
//收款1件
function gatheringOne(id){
	if(!id){
		return;
	}
...
//付款
function doGathering(ids){
	
		$.ajax({
			type:"POST",
			async:false,  //默认true,异步
			data:{ids:ids},
			url:"<%=request.getContextPath()%>/mall/manage/sellRecord/gathering.action",
			success:function(data){
				if(data=='SUCCESS'){
					search();
						dialog.alert("收款成功。");
...
<div class="content">
    ...
    <div class="right_lesson"  style="">
        ...
        <h4 >课程销售总金额：¥<span id="totalMoney"></span>   课程总销量：<span id="totalRecord"></span>个 已收款金额：¥<span id="payMoney"></span></h4>
        </div>
        <div class="button_gr">
              <input type="button" value="导出记录" onclick="exportToExcel()"/>
            <input type="button" value="批量收款" onclick="gathering()"/>
        </div>
        <div class="search_3">
         	<p>   
