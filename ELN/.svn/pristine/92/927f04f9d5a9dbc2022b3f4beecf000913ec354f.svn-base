...
function fillUserInfo(){
	...
	$("#postName").val(getZTreePathNameWithRootNode($.fn.zTree.getZTreeObj("postTreePage").getNodeByParam("id", user.postId, null),'name'));
	$("#deptName").val(getZTreePathName($.fn.zTree.getZTreeObj("treePage").getNodeByParam("id", user.deptId, null),'name'));
	$("#mobile").val(user.mobile);
	$("#name").val(user.name);
	$("#userName").val(user.userName);
	$("#address").val(user.address);
	$.each($("input[name='isManager']"), function(index, val){
		if($(val).val() == user.isManager){
			val.checked = true;
		}
	});
	if(user.birthDay != ''){
	...
	$.each($("input[name='role']"), function(index, val){
		$.each(user.roleList, function(index_, val_){
			if($(val).val() == val_.id){
				val.checked = true;
			}
		});
	...
	$('#addForm').isValid(function(v) {
		if(v){
			...
			$.ajax({
				...
				async:false,  //默认true,异步
				data:param,
				url:"<%=request.getContextPath()%>/manageUser/updatePulianUser.action",
				success:function(data){
					if(data == 'SUCCESS'){
						dialog.alert("修改用户成功。",function(){cancel();});
					}
				}
			});
		}else{
...
<div class="content">
	...
	<div style="padding-bottom: 5px;border-bottom: 1px solid #cccccc; margin-bottom: 10px;">
		...
		<span style="  position: relative;top: 5px; color: #3a3a3a; font-size: 16px; font-weight: bold;">修改学员</span>
	</div>
	<form id="addForm">
        ...
        <div class="add_gr">
    	 ...
    	 <div class="add_gr">
        	<div class="add_fl">
             ...
             <div class="add_fr">
            	<input type="text" id="deptName" disabled name="deptName"/>&nbsp;&nbsp;<input type="button" value="选择部门" onclick="chooseDept()"/>
            </div>
    	</div>
    	 <div class="add_gr">
        	<div class="add_fl">
             ...
             <div class="add_fr">
            	<input type="text" id="postName" disabled name="postName"/>&nbsp;&nbsp;<input type="button" value="选择岗位" onclick="choosePost()"/>
            </div>
    	</div>
    	 <div class="add_gr">
        	<div class="add_fl">
