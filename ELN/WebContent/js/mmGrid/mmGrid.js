    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            if(opts.checkCol){
                ...
                opts.cols.unshift({title:chkHtml,width: 22, align: 'center' ,lockWidth: true, checkCol: true, renderer:function(){
        ...
        , _initHead: function(){
            ...
            $.each($ths,function(index){
                ...
                allWidth += expandCols[index].width;
            });
            
            $.data($head,'all-width',allWidth);
        ...
        , _fullWidthRows: function(){
            ...
            //如果 $mmGrid.width() 小于  $.data($head,"all-width")，不作变化
            if($mmGrid.width() - $.data($head,"all-width") < 0){
            	return;
            }
        ...
        , _loadAjax: function(args){
            ...
            $.ajax({
                ...
                try{
                    ...
                    if($.isArray(data[opts.root])){
                        ...
                        //wj add 自动判断 翻页
                        var nowPage = parseInt(params[pageOpts.pageParamName], 10);   //当前页，从1开始
                        var nowPageSize = parseInt(params[pageOpts.limitParamName], 10);   //当前页，一页的数据量
                        var nowTotal = parseInt(data[pageOpts.totalCountName], 10);   //后台返回的总数据量

                        if(nowTotal !=0 && (nowPage-1)*nowPageSize >= nowTotal){
                        	//前一页 允许的最大数据 已经大于 总量
                        	//加载数据允许的最后一页
                        	var newPage = parseInt(nowTotal/nowPageSize, 10);
                        	if(nowTotal%nowPageSize != 0){
                        		newPage + 1;
                        	}
                        	
                        	params[pageOpts.pageParamName] = newPage;
                        	that._loadAjax(params);
                        	return;
                        }
                    }
            ...
            //wj add 去除全选  2015-10-29
            this.$head.find(".checkAll").prop("checked", false);
        }
