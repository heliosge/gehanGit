...
function save(){
	...
	if(selectRows.length > 0){
		var flag = true;
		$.each(window.parent.$("input[name='checkUserId']"),function(index, val){
			if($(val).val() == selectRows[0].id){
				dialog.alert("该人员已在审批流程中，请重新选择。");
				flag = false;
			}
		})
		if(!flag){
		...
		window.parent.$('#checkUserName_'+window.parent.inputId).val(selectRows[0].name);
		window.parent.$('#checkUserId_'+window.parent.inputId).val(selectRows[0].id);
	}
