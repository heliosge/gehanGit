...
#detail .de_con .con_top h4 {    float: left; width:308px;}

#detail .de_con .con_top p {
    float: left;
}
...
.grid_style {border:solid #e4e4e4; border-width:1px 0px 0px 1px;width:100%}
	        ...
	        //indexCol: true,  //索引列
	     	cols: [{title: 'ID', name: 'id', width:30, hidden:true},
				   ...
				   {title: '通过条件', name: 'content', width:120, align:'center', renderer:function(val, item, rowIndex){
					   ...
					   return (val.trainType == 1 ?'完成课程/'+(item.isPass == 1 ?'已通过':"未通过"):  "学习成绩不少于"+val.passPercent+"分/"+(item.isPass == 1 ?'已通过':"未通过"));
				   }},
				   {title: '操作', name: 'content', width:120, align:'center', renderer:function(val, item, rowIndex){
					   return val.trainType == 1 ?'<a href="javascript:void(0);" style="color:#999;">我要学习</a>':'——';
					   }}
	           ]
	    });
	...
	<div id="detail">
		...
		<div class="de_con" style="border:none;width:1046px;padding-left: 0px;">
			...
			<div class="con_top" style="width:1046px;">
				...
				<div style="width:50%;float:right;border:1px solid #cccccc;padding: 0 19px;">
					<h4 title="${arrange.name }" class="arrangeNameStyle">${arrange.name }</h4>
					<p style="width: 200px;float: right;padding-top: 27px;">目前已有${arrange.joinedNum }人报名</p>
					<div style="border-bottom: 1px solid #cccccc;float:left;width:500px;">
					</div>
					<p>&nbsp;</p>
								...
								<td>
									...
									<c:choose>
					                	<c:when test="${item.status==1 }">
					                		<a href="javascript:void(0);" style="color:#999;">进入</a>
					                	</c:when>
					                	<c:otherwise>
					                		<a href="javascript:void(0);" style="color:#999;">进入</a>
					                	</c:otherwise>
