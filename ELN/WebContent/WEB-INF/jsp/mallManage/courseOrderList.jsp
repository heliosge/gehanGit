...
$(function(){
	...
	$('#exampleTable').mmGrid({
		...
		url:"<%=request.getContextPath()%>/mall/manage/course/order/list.action",
        ...
        params: function(){
     	...
     	cols: [{title: 'ID', name: 'id', width:30, hidden:true},
     	      {title:'订单编号',name:'code',width:30,align:'center'},
 			   {title: '课程名称', name: 'names', width:60, align:'center', renderer:function(val, item, rowIndex){
 				   var valStr=val;
 				   var htmlStr='';
 				   ...
 				   if(item.relateIds){
 					  var ids=item.relateIds.split(',');
 	 				   for(var i=0;i<ids.length;i++){
 	 					  htmlStr+='<a href="<%=request.getContextPath()%>/mall/manage/toCourseDetailPage.action?id='+ids[i]+'" >'+nameArr[i]+'</a>';
 	 				   }
 				   }**/
			   ...
			   {title:'订单价格',name:'price',width:30,align:'center'},
 			   {title: '买家企业', name: 'companyName', width:30, align:'center'},
 			   {title: '卖家企业', name: 'sellCompanyName', width:30, align:'center',renderer:function(val,item,rowIndex){
 				  var valStr=val;
				   var htmlStr='';
				   if(valStr&&valStr.length>10){
					  valStr = valStr.substring(0,10)+'...';
 			   ...
 			   {title: '下单时间', name: 'createTime', width:85, align:'center'},
 			   {title: '状态', name: 'status', width:25, align:'center',renderer:function(val,item,rowIndex){
 			     ...
 			     {title: '发票状态', name: 'invoiceStatus', width:25, align:'center',renderer:function(val,item,rowIndex){
 				 var str='';
 				 ...
 				 return str;
				   ...
				   {title: '操作', name: 'status', width:100, align:'center', renderer:function(val, item, rowIndex){
...
window.open("<%=request.getContextPath()%>/mall/manage/exportCourseOrder.action?"+paramStr);
...
<div class="content">
	...
	<div style="padding-bottom: 5px;border-bottom: 1px solid #cccccc; margin-bottom: 10px;">
		...
		<span style="  position: relative;top: 5px; color: #3a3a3a; font-size: 16px; font-weight: bold;">课程订单管理</span>
	 </div>
