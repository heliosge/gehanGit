...
function getQuestions(){
	$.ajax({
		...
		url: "<%=request.getContextPath()%>/myQuestionnaire/getAnswerDetail.action",
...
//返回调查汇总统计页面
function backList(){
	if(backType == 'questionnaireList'){
		window.location.href = "<%=request.getContextPath()%>/myQuestionnaire/toList.action";
	}else if(backType == 'resultList'){
		window.location.href = "<%=request.getContextPath()%>/questionnaire/toResultToal.action?id="+iBean.id+"&backType=detail";
	}
}
