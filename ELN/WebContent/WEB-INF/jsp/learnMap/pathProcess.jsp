...
function renderTable(i){
	...
	$('#exampleTable_'+i).mmGrid({
        ...
        params:function(){
        	...
        	param.deptName = $.trim($("#deptName_"+i).val());
        ...
        cols: [{title: '姓名', name: 'userName', width:30},
			   ...
			   {title: '考试是否通过', name: 'isPassed', width:40,renderer:function(val,item,rowIndex){
				   if(val==1){
					   return '通过';
				   }else{
					   return '未通过';
			   ...
			   {title: '操作', name: 'id', width:40,renderer:function(val,item,rowIndex){
			      ...
			      var htm="";
			       if(item.promotionStatus==1){
			    	   ...
			    	   htm+="<a href='javascript:void(0);' style='color: gray;' >晋升完成</a>";
			       }else{
			    	   htm="<a href='javascript:void(0);' onclick='doAgree("+val+","+i+",4)' >同意晋升</a>";
			       }
				   return htm;
			   }}
           ],
...
function doAgree(id,i,status,reason){
	$.ajax({
		type:"POST",
		asysc:true,
		data:{"id":id,"status":status,"reason":reason},
		dataType:"json",
		url:"<%=request.getContextPath()%>/map/agreePromotion.action",
		success:function(data){
			if(data=="SUCCESS"){
				dialog.alert('晋升成功！')
				search(i);
			}else if(data==false){
				dialog.alert('不满足晋升条件！')
			}else{
				dialog.alert('晋升失败！')
			}
		}
	...
	<div id="course_all">
            <div class="notes_list fl">
            	...
            	<div style="padding-bottom: 5px;border-bottom: 1px solid #cccccc; margin-bottom: 10px;">
					...
					<span style="  position: relative;top: 5px; color: #3a3a3a; font-size: 16px; font-weight: bold;">查看学员</span>
				</div>
            	<% 
            	...
            	<div class="map" id="map_<%=i+1 %>">
                    ...
                    <div class="search_3"  style="display:none">
                        <span>部门：</span><input id="deptName_<%=i+1%>"  type="text" />
