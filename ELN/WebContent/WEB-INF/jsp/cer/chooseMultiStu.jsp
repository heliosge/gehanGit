...
<style type="text/css">
	...
	.ztree li span.button.noline_open{position: relative;margin-right: -18px;background-image:url("<%=request.getContextPath()%>/images/img/ico_sq.png")}
	.ztree li span.button.noline_close{position: relative;margin-right: -18px;background-image:url("<%=request.getContextPath()%>/images/img/ico_zk.png")}
...
function save(){
	...
	if(selectRows.length > 0){
		for(var i=selectRows.length-1;i>=0;i--){
			for( var j=0;j<window.parent.receiveUserId.length;j++){
				if(window.parent.receiveUserId[j] == selectRows[i].id){
	...
	if(selectRows.length > 0){
		for(var i=selectRows.length-1;i>=0;i--){
			window.parent.receiveUserId.push(selectRows[i].id);
			name += selectRows[i].name+" ";
		}
		window.parent.$('#receiveUserName').val(window.parent.$('#receiveUserName').val()+name);
	}
