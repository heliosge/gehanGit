...
$(function(){
	//表格
	var mmk = $('#knowledgeTable').mmGrid({
		    	root:'rows',// 和后台数据保存列一致,对应MMGridPageVoBean rows
				url:"<%=request.getContextPath()%>/approve/getKnowlegeListByPL.action",
		    	width: 'auto',
		    	height: 'auto',
		        ...
		        cols: [{title: 'ID', align:'center',name: 'knowledgeId', width:60, hidden:true},
					   ...
					   {title: '操作', align:'center',name: 'knowledgeId', width:60, renderer:function(val, item, rowIndex){
						   var buttonStr = '<span class="span_btn" href="#" onclick="detailKnowledge('+val+')" >查看</span>';
							   if(item.shareStatus==5){
	...
	<%-- var mmc = $('#exampleTable').mmGrid({
    	root:'rows',// 和后台数据保存列一致,对应MMGridPageVoBean rows
		url:"<%=request.getContextPath()%>/approve/getApproveCourseListByPL.action",
    	width: 'auto',
    	height: 'auto',
    	fullWidthRows: true, //表格第一次加载数据时列伸展，自动充满表格
    	showBackboard: false,
        ...
        cols: [{title: 'ID', align:'center',name: 'id', width:60, hidden:true},
			   ...
			   {title: '操作', align:'center',name: 'id', width:60, renderer:function(val, item, rowIndex){
				   var buttonStr = '<span class="span_btn" href="#" onclick="detailCourse('+val+')" >查看</span>';
					   if(item.shareStatus==5){
	...
	$.fn.zTree.init($("#companyTree"), treeSet,${companyList});//初始化模型树
	    ...
	    <div class="content_2" style="display: block;">
	        <div class="left_tree">
	           ...
	           <%--  <p class="sch">
	            ...
	            <div id="companyTree" class='ztree tree'style="height: 480px;overflow: auto;">
	            </div>
	...
	function jzData(index){
		...
		if(index==0){
		    ...
		    mmt = $('#topicTable').mmGrid({
		    	root:'rows',// 和后台数据保存列一致,对应MMGridPageVoBean rows
				url:"<%=request.getContextPath()%>/approve/getApproveTopicListByPL.action",
		    	width: 'auto',
		    	height: 'auto',
		    	fullWidthRows: true, //表格第一次加载数据时列伸展，自动充满表格
		    	showBackboard: false,
		        ...
		        cols: [{title: 'ID', align:'center',name: 'id', width:60, hidden:true},
					   ...
					   {title: '操作', align:'center',name: 'id', width:60, renderer:function(val, item, rowIndex){
						   var buttonStr = '<span class="span_btn" href="#" onclick="detailTopic('+val+')" >查看</span>';
							   if(item.shareStatus==5){
