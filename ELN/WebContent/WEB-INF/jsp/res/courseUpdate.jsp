...
function fillCourseInfo(){
	...
	var groupName = '';
	var deptName = '';
	for(var i = 0; i < course.deptList.length; i++){
		deptName += "部门："+course.deptList[i].name+"； ";
		deptIds.push(course.deptList[i].id);
	}
	...
	$('#addForm').isValid(function(v) {
		if(v){
			$.ajax({
		 		...
		 		success:function(data){
		 			if(data=='SUCCESS'){
		 				window.location.href="<%=request.getContextPath()%>/res/toUpdateResSectionPage.action?courseId="+course.id+"&sectionCount=0";
		 			}else{
		 				dialog.alert("修改失败！");
		 			}
		 	    }
	...
	<div class="lesson_add">
     ...
     <!--    <div class="add_gr">
        ...
        <div class="add_gr"  style="height:auto;">
            ...
            <div class="add_fr">
					<p class="tp">
						<img id="previewPath" src="" width="124px" height="86px"/>
					</p>
            </div>
        </div>
        <div class="add_gr">
