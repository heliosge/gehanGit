...
function save(){
	window.parent.$("#userSpan").show();
	var selectRows = $('#exampleTable').mmGrid().selectedRows();
	if(selectRows.length > 0){
		for(var i=selectRows.length-1;i>=0;i--){
			for( var j=0;j<window.parent.receiverIds.length;j++){
				if(window.parent.receiverIds[j] == selectRows[i].id){
	...
	for(var i = 0; i < selectRows.length; i++){
		window.parent.receiverIds.push(selectRows[i].id);
		window.parent.$('#userSpan').append(selectRows[i].name+";");
	}
