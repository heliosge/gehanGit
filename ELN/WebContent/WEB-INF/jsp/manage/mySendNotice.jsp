...
$(function(){
	...
	$('#exampleTable').mmGrid({
		...
		url:"<%=request.getContextPath()%>/manageUser/selectMySendNoticeList.action",
        ...
        params: function(){
     	...
     	cols: [{title: 'ID', name: 'id', width:30, hidden:true},
               {title: '序号', name: 'id', width:60, hidden:true, align:'center'},
 			   {title: '邮件标题', name: 'title', width:120, align:'center', renderer:function(val, item, rowIndex){
				   return '<a href="sendNoticeDetail.action?id='+item.id+'">'+val+'</a>';
			   }},
 			   {title: '收件人', name: 'recUserName', width:60, align:'center', renderer:function(val, item, rowIndex){
				   return '<h4 class="recUserStyle" title="'+val+'">'+val+'</h4>';
			   }},
...
function sendNotice(){
	window.location.href="<%=request.getContextPath()%>/manageUser/toSendMessagePage.action";
}
...
<div id="content_i">
    ...
    <div class="right_options" id="rop">
        <div class="right_option5" style="padding-bottom:0px;">
            <input type="button" value="发站内信" class="btn_10" onclick="sendNotice()"/>
             <input type="button" value="批量删除" class="btn_10" onclick="deleteRows()"/>
            <p>邮件标题：<input type="text" id="title"/>收件人：<input type="text" /></p>
