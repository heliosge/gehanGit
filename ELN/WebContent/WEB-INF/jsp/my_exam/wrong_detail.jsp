...
$(function(){
	$.ajax({
		...
		data:{"id":wrongId},
		url: "<%=request.getContextPath()%>/myExamManage/getWrongQuestionDetail.action",
		success:function(data){
			if(data != null){
				var userAnswerBean = data.userAnswerBean;
				...
				html += '<em><a href="javascript:void(0);" onclick="deleteWrong('+wrongId+','+userAnswerBean.id+')">移出错题集</a></em></h5>';
				if(type == 6){//组合题
...
/*移出错题集  */
function deleteWrong(wrongId,userAnswerId){
	dialog.confirm('确认移出吗？', function(){
		$.ajax({
			type:"POST",
			async:true,  //默认true,异步
			data:{"id":wrongId,"answer_id":userAnswerId},
...
//返回错题集列表
function backList(){
	window.location.href = "<%=request.getContextPath()%>/myExamManage/gotoWrongCard.action";
}
</script>
...
<div class="notes_list" style="padding-bottom: 0px;width:1406px;">
	...
	<div class="on_test" style="margin-top:12px;">
        ...
        <div class="test_tool">
            ...
            <div class="tool_bottom">
            	<input type="button" value="显示全部" class="input_a" />
    	...
    	<div class="left_test" id="wrongDiv" style="padding-top:0px;">
        	<!-- <div class="dx">
            	<h5><span>(1分)</span>1.全新QX250具备灵活的车内空间组合，其后排座椅放倒比例为：<em><a href="#">移出错题集</a></em></h5>
           ...
           <!--  <div class="dx">
            	<h5><span>(3分)</span>2.全新QX250具备灵活的车内空间组合，其后排座椅放倒比例为：<em><a href="#">移出错题集</a></em></h5>
