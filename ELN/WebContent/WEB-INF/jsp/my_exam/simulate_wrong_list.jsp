...
$(function(){
	$("#userId").val(userId);
	/* $('#ul_exam').find('li').each(function(index,element){
		if($(this).attr("id") == wrongType){
	...
	$('#examTable').mmGrid({
		...
		url:'<%=request.getContextPath()%>/myExamManage/getSimulateWrongCard.action',
		width: '1046px',
		height: 'auto',
...
function viewWrongQuestion(wrongId){
	window.location = '<%=request.getContextPath()%>/myExercise/gotoExerciseWrongDetail.action?id='+ wrongId+"&wrongType=2";
}
...
<input type="hidden" id="wrongType" name="wrongType" value="2"/>
<div id="course_all">
